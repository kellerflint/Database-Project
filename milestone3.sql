...
# Selects all actors who have played a role in the Lord of the Rings Franchise (Using inner join)
SELECT actor_lname, actor_fname, role_lname, role_fname FROM Actor, Role, Movie
	WHERE (Actor.actor_id = Role.actor_id
    	AND Role.movie_id = Movie.movie_id)
    	AND (Movie.movie_name LIKE 'Lord of the Rings%')
    	GROUP BY Actor.actor_id;

# Selects the number of movies all directors have directed (Using aggregate functions (COUNT))
SELECT director_lname, director_fname, COUNT(Movie.director_id) AS NumDirectedMovies FROM Director, Movie
	WHERE Director.director_id = Movie.director_id
	GROUP BY Director.director_id;

# Selects all actors who have no movies associated with him/her in the database (Using subqueries)
INSERT INTO Actor VALUES
	('ACT-RS-000', 'Schneider', 'Ron', '1963-10-31', 'M', 'San Franisco, California, United States', 'Gigolo');
SELECT Actor.actor_id, actor_lname, actor_fname FROM Actor
	WHERE NOT EXISTS
		(SELECT Role.actor_id FROM Role
			WHERE Role.actor_id = Actor.actor_id);

# Selects all actors a director is associated with (Using GROUP BY clause)
SELECT Director.director_lname, Director.director_fname, actor_lname, actor_fname FROM Actor, Role, Movie, Director
	WHERE Actor.actor_id = Role.actor_id
    	AND Role.movie_id = Movie.movie_id
    	AND Movie.director_id = Director.director_id
    	GROUP BY Actor.actor_id;

# Selects all genre and lists all movies associated with that generes (using outer join)
select genre_name, movie_name from Genre left outer join Movie on Genre.genre_id = Movie.genre_id;